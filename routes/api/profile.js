...
router.get('/:username', (req, res) => {
  ...
  if (typeof username === 'undefined' || username.length === 0) {
    response = {
      ...response,
      profile: 'Tên người dùng là bắt buộc.'
    };
    error = true;
  }
  ...
  connection.query(sql, [username, username], (err, result) => {
    ...
    if (result.length === 0) {
      response = {
        ...response,
        profile: 'Người dùng không tồn tại.'
      };
      error = true;
    }
          ...
          connection.query(sql, [user.id, result[0].id, result[0].id, user.id], (err, result5) => {
            ...
            connection.query(sql, [user.id, result[0].id, result[0].id, user.id], (err, result5) => {
              ...
              connection.query(sql, [result[0].id], (err, result6) => {
                ...
                let formatted = hd(diff * 1000, { language: 'vi', round: true, largest: 1 });
                let message = ((status === 'online')  
                ? "Đang trực tuyến"  
                : ((result6[0].time === null)  
                    ? "Chưa bao giờ online" 
                    : `Ngoại tuyến từ ${formatted}`));  
